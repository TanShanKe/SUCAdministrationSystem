...
$sql2 = "SELECT MAX(YEAR(applicationDate)) AS latestYear, CASE WHEN MONTH(MAX(applicationDate)) BETWEEN 3 AND 5 THEN 1 WHEN MONTH(MAX(applicationDate)) BETWEEN 6 AND 9 THEN 2 WHEN MONTH(MAX(applicationDate)) BETWEEN 10 AND 12 THEN 3 WHEN MONTH(MAX(applicationDate)) BETWEEN 1 AND 2 THEN 4 ELSE 1 END AS latestSem FROM leave_record";
...
<div class="container-fluid" style="width: 90%;" >
    ...
    <table class="table "> 
        ...
        if ($selectedSem == 1) {
          ...
          $endMonth = 2;   // February
          $sql = "SELECT lecturerSignature, leave_record.applicantID, leave_record.leaveID, leave_record.applicationDate, leave_record.aaroSignature FROM leave_subject LEFT JOIN leave_record on leave_subject.leaveID=leave_record.leaveID 
          WHERE 
